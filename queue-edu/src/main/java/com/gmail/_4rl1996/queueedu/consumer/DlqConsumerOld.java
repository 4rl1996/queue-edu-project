...
public class DlqConsumerOld {
    ...
    @QueueBinding(
            value = @Queue(name = "${rabbit.properties.dlqEduQueueKeyName}"),
    ...
    public void onMessage(Message<TestMessage> message) {
        ...
        System.out.println("\n\nMessage was received by DlqConsumerOld from queue-edu service, queue = " + applicationProperties.getDlqEduQueueKeyName() +
                ".\nMessageHeaders are:\n" +
                message.getHeaders() +
                "\nMessage Payload is:\n" +
                payload);
