...
public class ConsumerOld {
    ...
    @QueueBinding(
            value = @Queue(name = "${rabbit.properties.queueEduName}",
                    arguments =
                            {@Argument(name = "x-dead-letter-exchange", value = "${rabbit.properties.dlqEduExchangeName}"),
                                    @Argument(name = "x-dead-letter-routing-key", value = "${rabbit.properties.dlqEduQueueKeyName}")}
            ),
